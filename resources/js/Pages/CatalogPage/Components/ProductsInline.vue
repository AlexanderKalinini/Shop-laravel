...
<template>
  <div class="products grid grid-cols-1 gap-y-8">
    <span v-if="products?.length == 0">Ничего не найдено</span>
    <div
    ...
    >
      ...
      <div class="grow flex flex-col py-8 px-6 md:px-8">
        ...
        <div
          class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6 mt-6"
        >
          <div class="flex items-baseline gap-4">
            <div
              class="hover:text-pink text-md xl:text-lg font-black"
              :class="{ 'text-pink': product.old_price }"
            >
              {{ product.price }} ₽
            </div>
            <div
              v-if="product.old_price"
              class="text-body text-sm xl:text-md font-semibold line-through"
            >
              {{ product.old_price }} ₽
            </div>
          </div>
          <div class="flex flex-wrap items-center gap-4">
            <button
              @click="callProductToCart(product, 1)"
            ...
            <button
              @click="
                favorit.setFavorit(product), sessionStorageEvent('favorit')
              "
